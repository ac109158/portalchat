...
if (window.angular) {
    ...
    angular.module('chatModule', ['firebase', 'ngSanitize', 'emoji', 'chatModule.filters', 'chatModule.services', 'chatModule.factories', 'chatModule.directives', 'chatModule.controllers', 'LocalStorageModule']).config(function($logProvider) {
        $logProvider.debugEnabled(false);
    }).config(['$httpProvider', function($httpProvider) {
        $httpProvider.defaults.timeout = 5000;
    }]).constant('ENCRYPT_PASS', 'Anything but plain text').constant('FBURL', database).constant('EXT_LINK', external).run(function($window, $rootScope) {
    ...
    angular.module('chatModule.controllers', []).controller('MainController', ['$rootScope', '$scope', '$log', 'UtilityService', 'NotificationService', 'UserService', 'BrowserService', 'states', '$timeout', 'localStorageService', function($rootScope, $scope, $log, UtilityService, NotificationService, UserService, BrowserService, states, $timeout, localStorageService) {
        $scope.runApp = false;
        $scope.showProfileMenu = false;
        $scope.showPresenceMenu = false;
        $scope.showVolumeMenu = false;
        $scope.sound_level = 2;
        UserService.profile($scope);
        $timeout(function() {
        ...
        jQuery(document).ready(function() {
            var check_for_user = setInterval(function() {
                ...
                if (UserService._user_profile && UserService._user_profile.user_id && angular.isDefined(UserService._user_profile_location) && UserService._user_init) {
                    ...
                    if (UserService._user_profile.mc) {
                        $scope.mc_group_chat = DirectoryChatService.__buildNewDirectoryChat($scope, ('mc_' + UserService._user_profile.mc.user_id + '_group_chat'), UserService._user_profile.mc.name + ' - MC Team Chat', UserService._user_profile.mc.user_id, false, false, 3);
        ...
        $scope.setLayout = function() {
            ...
            if ($scope.isChatModuleOpen && $scope.isPageLoaded) {
                if ($scope.layout == 1 || $scope.layout == null) {
                    $scope.chat_module_directory_height = parseInt($scope.windowHeight, 10) || 0;
                    $scope.chat_module_header_height = parseInt(document.getElementById("cm-main-panel-header").clientHeight, 10) || 0;
                    $scope.cm_directory_chat_height = $scope.chat_module_directory_height - $scope.chat_module_header_height;
                    $scope.cm_directory_upper_panel = parseInt(document.getElementById("cm-main-panel-upper-panel").clientHeight, 10) || 0;
                    $scope.cm_directory_chat_height = $scope.cm_directory_chat_height - $scope.cm_directory_upper_panel;
                    $scope.chat_module_nav_height = parseInt(document.getElementById("cm-main-panel-nav").clientHeight, 10) || 0;
                    ...
                    $scope.chat_module_header_height = parseInt(document.getElementById("cm-main-panel-header").clientHeight, 10) || 0;
                    $scope.chat_module_upper_panel_height = parseInt(document.getElementById("cm-main-panel-upper-panel").clientHeight, 10) || 0;
                    $scope.chat_module_resize_slider_height_2 = parseInt(document.getElementById("cm-main-panel-resize-slider-2").clientHeight, 10) || 0;
                    ...
                    if ($scope.activeChats.length > 0) {
                        ...
                        $scope.cm_directory_upper_panel = parseInt(document.getElementById("cm-main-panel-upper-panel").clientHeight, 10) || 0;
                        $scope.cm_directory_chat_height = $scope.cm_directory_chat_height - $scope.cm_directory_upper_panel;
                        $scope.cm_directory_tracker_height = parseInt(document.getElementById("cm-main-panel-tracker-3").clientHeight, 10) || 0;
                        ...
                        $scope.chat_module_resize_slider_height = parseInt(document.getElementById("cm-main-panel-resize-slider").clientHeight, 0) || 0;
                        $scope.cm_directory_chat_height = $scope.cm_directory_chat_height - $scope.chat_module_resize_slider_height;
                        $scope.chat_module_resize_slider_height_width = parseInt(document.getElementById("cm-main-panel-resize-slider-width").clientHeight, 0) || 0;
        ...
        $scope.updateChatSettings = function(chat) {
            UserService._user_settings_location.child(chat.index_position).update({
        ...
        $scope.setActiveChatsPriority = function() {
            ...
            angular.forEach($scope.activeChats, function(val, key) {
                ...
                ChatService._active_sessions_user_location.child(val.to_user_id || val.session_key).update({
                    'index_position': $scope.active_chats_index
                });
            });
        }
        $scope.updateChatResize = function(chat) {
            ChatService._active_sessions_user_location.child(chat.to_user_id || chat.session_key).update({
        ...
        $scope.updateTag = function(chat) {
            if (angular.isDefined(chat.tag_description) && chat.tag_description.length > 2 && chat.tag_description.length <= 20) {
                ...
                $scope.clear(chat);
                chat.tag_description = String(chat.tag_description).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
                if (chat.isgroupChat) {
        ...
        $scope.addDirectoryTopic = function(chat) {
            if (angular.isDefined(chat.topic_description) && chat.topic_description.length > 5 && chat.topic_description.length <= 36) {
                chat.topic_description = String(chat.topic_description).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
                chat.isTextFocus = true;
                if (angular.isDefined(chat.firebase_location)) {
                    chat.firebase_location.update({
                        topic: chat.topic_description
        ...
        $scope.updateTopic = function(chat) {
            var new_topic = String($('#topic_' + chat.session_key).text()).replace(/&/g, '').replace(/</g, '').replace(/>/g, '').replace(/"/g, '');
        ...
        $scope.updateDirectoryTopic = function(chat) {
            var new_topic = String($('#topic_' + chat.session_key).text());
            if (new_topic == 'null' || new_topic == '' || new_topic.length < 5) {
                return false;
            }
            if (angular.isDefined(chat.firebase_location)) {
            ...
            if (angular.isUndefined(chat.chat_text) || chat.chat_text == null) {
                ...
                $timeout(function() {
                    var toFirekey = chat.to_user_message_location.push({
                        author: UserService._user_profile.user_id,
                        authorName: UserService._user_profile.name,
                        to: chat.to_user_id,
                        text: chat_text,
                        encryption: UserService.encryption,
        ...
        $scope.addGroupChatMessage = function(chat) {
            if (angular.isUndefined(chat.chat_text) || chat.chat_text == null) {
                ...
                var firekey = chat.group_message_location.push({
                    author: UserService._user_profile.user_id,
                    authorName: UserService._user_profile.name,
                    avatarId: UserService._user_profile.avatar,
                    encryption: UserService.encryption,
                    reference: chat.reference,
                    referenceAuthor: chat.referenceAuthor,
                    referenceName: chat.referenceName,
        ...
        $scope.getAuthorFirstName = function(message) {
            if (message.authorName) {
                var name_split = message.authorName.match(/\S+/g);
        ...
        $scope.sendChatReceipt = function(chat) {
            ...
            if (chat.isGroupChat) {
                ...
                angular.forEach(chat.participant_log, function(value, key) {
                    this.push(value);
                }, chat_record.participants);
                angular.forEach(chat.group_chats, function(value, key) {
                    this.push({
                ...
                angular.forEach(chat.chats, function(value, key) {
                    this.push({
                        author: value.authorName,
                        time: Math.ceil(value.time / 1000),
                        text: value.text,
        ...
        $scope.showInGeneralDirectory = function(user) {
        ...
        $scope.alertNewChat = function(index, internal, message) {
            ...
            if (internal) {
                ...
                if ((!ChatService._is_playing_sound) && angular.equals($scope.activeChats[index].isSound, true)) {
                    ...
                    $timeout(function() {
                        $scope.resetTyping($scope.activeChats[index]);
                        NotificationService.__playSound(NotificationService._chat);
                        ChatService._is_playing_sound = false;
                    }, 50);
                }
                    ...
                    $timeout(function() {
                        $scope.$evalAsync(function() {
                            ...
                            function resizeChild() {
                                $timeout(function() {
                                    $scope.externalWindowObject.document.documentElement.style.overflow = 'hidden';
                                    $scope.externalWindowObject.document.body.scroll = "no";
        ...
        $scope.addInvitee = function(chat) {
            ...
            if ($scope.filteredArray && chat.invitee_set == true) {
                ...
                chat.invited = '';
                chat.user_log[$scope.invited_user.user_id] = {
                ...
                if (chat.isGroupChat == false) {
                    ...
                    $timeout(function() {
                        ...
                        if (angular.isDefined(chat.index_position) && $scope.activeChats[chat.index_position].session_key == chat.session_key) {
                            ...
                            index = chat_log.indexOf(chat.session_key);
                        }
                        if (index > -1) {
                        ...
                        $timeout(function() {
                            ...
                            angular.forEach(new_group_chat.user_details, function(val, key) {
                                if (angular.isDefined(key) && key !== null) {
                                    if (angular.isDefined(new_group_chat.user_details[key]) && angular.isDefined(new_group_chat.user_details[key].session_location)) {
                                        new_group_chat.user_details[key].session_location.update({
                                            groupChat: new_group_chat.isGroupChat,
                                            name: new_group_chat.chat_description,
                                            session_key: new_group_chat.session_key,
                                            admin: new_group_chat.admin,
                                            time: new_group_chat.time
                                        });
                                    } else {
                                        $log.debug('Session location for ' + key + ' : ' + angular.toJson(val) + ' was not defined.');
                                    }
                                };
                            });
                            ChatService.__deactivate_session_from_user_location(chat, $scope, false, false);
                            $scope.invited_user = undefined;
                    ...
                    if (angular.isDefined(chat.firebase_location)) {
                        ...
                        var d = new Date();
                        var time = d.getTime();
                        $scope.invited_user.session_location.update({
                            groupChat: chat.isGroupChat,
                            name: chat.chat_description,
                            session_key: chat.session_key,
                            admin: chat.admin,
                            'time': time
                        });
        ...
        $scope.selectActiveSelection = function(chat) {
            if (angular.isObject($scope.invited_user)) {
                ...
                $scope.filteredArray = $filter('filter')(chat.invite_list, {
                    name: chat.invited
                });
                $log.debug($scope.filteredArray);
    ...
    'use strict';
    angular.module('chatModule.services', []).service('BrowserService', ['$http', '$log', function($http, $log) {
        var BrowserService = {};
        (function(window) {
            {
                var unknown = '-';
    ...
    }).service("UserService", ['$log', '$http', '$firebase', 'FBURL', 'EXT_LINK', "ENCRYPT_PASS", '$window', 'BrowserService', 'NotificationService', '$timeout', 'localStorageService', function($log, $http, $firebase, FBURL, EXT_LINK, ENCRYPT_PASS, $window, BrowserService, NotificationService, $timeout, localStorageService) {
        var UserService = {};
        ...
        UserService._force_online = false;
            ...
            if (this.set == null) {
                ...
                $http({
                    ...
                    if (response.result == true) {
                            ...
                            UserService._user_state_location.child('state').on('value', function(snapshot) {
                                ...
                                if (user_state == 'Offline' && UserService._user_chat_presence && UserService._user_chat_presence.$value != 'Offline') {
                                    UserService._user_state_location.update({
                                        'state': UserService._user_stored_state
                                    });
                                } else {
                        ...
                        scope.last_mandatory_chat_index = $firebase(new Firebase(FBURL + UserService._users_reference + UserService._users_settings_reference + response.user_id + '/last-mandatory-chat/'));
                        ...
                        UserService._user_additional_profile_location.child('platform').update({
                            'ip': UserService._user_profile.ip
                        });
                        UserService._user_additional_profile_location.child('checkOutTime').once('value', function(snapshot) {
                            ...
                            if (lastCheckOut) {
                                if (lastCheckOut + 100000 < new Date().getTime()) {
                                    ...
                                    UserService._user_additional_profile_location.update({
        ...
        UserService.__requireRefresh = function() {
            if (UserService._user_profile.position == 34) {
                ...
                UserService._public_settings_location.update({
                    'require-refresh': true
                });
                $timeout(function() {
                    UserService._public_settings_location.update({
                        'require-refresh': false
                    })
                }, 250)
            } else {
        ...
        UserService.__watchProfiles = function(scope) {
            ...
            UserService._users_profiles_location.once('value', function(snapshot) {
            ...
            UserService._users_profiles_location.startAt().on('child_added', function(snapshot) {
        ...
        UserService.__watchOnline = function(scope, user) {
            ...
            UserService._users_online_location.child(user).on('value', function(snapshot) {
                ...
                if (online_status && online_status.online) {
                    UserService.__removeOfflineQueue(user_id);
                    UserService._online_users_list[user_id] = UserService._users_profiles_obj[user_id];
                    delete UserService._offline_users_list[user_id]
                } else {
        ...
        UserService.__getUserArray = function() {
            ...
            angular.forEach(UserService._users_profiles_obj, function(value, key) {
                this.push(value);
        ...
        UserService.detectLayout = function() {
        ...
        UserService.__setGeolocation = function(user) {
            if (navigator.geolocation) {
                ...
                UserService._users_geolocation_location.child(user).update({
                    lat: false,
                    lng: false
                });
        ...
        return UserService;
    }]).service('AudioService', ['$http', '$log', '$sce', function($http, $log, $sce) {
        var promise;
        var keyCheck;
        var AudioService = {
            audioHistory: [],
        ...
        OnlineService.__updateOnlineTimestamps = function(scope) {
            OnlineService._online_check_location.once('value', function(snapshot) {
                ...
                angular.forEach(user_timestamps, function(value, user_key) {
                    if (OnlineService._isTimestamps) {
                        ...
                        if (UserService._users_profiles_obj['user_' + user_key] && value > UserService._users_profiles_obj['user_' + user_key].last_check_in) {} else {
        ...
        var ChatService = {};
        ChatService._is_typing_presence = true;
        ChatService._header_color = "#00335B";
        ChatService._closed_header_alert_color = '#ce6000';
        ChatService._open_header_alert_color = '#4787ED';
        ChatService._self_reference = "Me";
        ChatService._parent_category_reference = "Chat-System/Users" + '/';
        ChatService._url_root = FBURL + ChatService._parent_category_reference;
        ChatService._chat_presence_states = ['Online', 'Offline', 'Busy', 'Invisible'];
        ChatService._active_session_reference = "Active-Sessions" + '/';
        ChatService._is_typing_reference = 'is-typing';
        ChatService._chat_message_storage_reference = "Stored-Messages/Users" + '/';
        ChatService._users_profiles_reference = FBURL + UserService._users_profiles_reference;
        ChatService._user_profiles_location = new Firebase(ChatService._users_profiles_reference);
        ChatService._parent_session_reference = 'Chats' + '/';
        ChatService._session_url_root = FBURL + ChatService._parent_session_reference;
        ChatService._group_parent_category_reference = "Chat-System/Group-Chats" + '/';
        ChatService._group_active_users_reference = "active-users";
        ChatService._group_url_root = FBURL + ChatService._group_parent_category_reference;
        ChatService._last_pushed_session = null;
        ChatService._last_query_location = '';
        ChatService._group_active_session_reference = "Active-Sessions" + '/';
        ChatService._group_message_location_reference = "user-messages" + '/';
        ...
        ChatService._message_load_size = 5;
        ChatService._message_fetch_size = 15;
        ChatService._storage_limit = false;
        ChatService._single_query_size = 500;
        ChatService._group_query_size = 5000;
        ChatService._store_time = 1209600000;
        ...
        ChatService.__set_active_sessions_user_location = function(scope) {
            ...
            ChatService._active_sessions_user_location.on('child_added', function(snapshot) {
                var data = snapshot.val();
        ...
        ChatService.__requestChatSession = function(scope, to_user, isfocus) {
            ...
            if (angular.isDefined(to_user.groupChat) && to_user.groupChat == true && angular.isDefined(to_user.session_key)) {
                ...
                if (scope.active_sessions[to_user.session_key] == true && angular.isDefined(scope.activeChats)) {
                    ...
                    angular.forEach(scope.active_sessions, function(value, key) {
                        this.push(value.key);
                    }, chat_log);
                ...
                ChatService.__buildGroupChatSession(scope, to_user, to_user.isOpen || true, isfocus, 1);
                return true;
            } else {
                ...
                ChatService.__buildChatSession(scope, to_user, to_user.isOpen, isfocus, 14);
                return true;
            }
        };
        ChatService.__setDefaultSettings = function(chat) {
            chat.user_log = {};
            chat.user_details = {};
            chat.participant_log = Array();
            chat.is_typing_list = Array();
            chat.chats = Array();
            chat.chat_log = Array();
            chat.group_chats = Array();
            chat.group_chat_log = Array();
            chat.isTopicFocus = false;
            chat.topic_description = '';
            chat.topic_height = 0;
            ...
            chat.referenceName = null;
            chat.referenceText = null;
            chat.isTypingPresence = ChatService._is_typing_presence;
            chat.isTyping = false;
            chat.invite = false;
            chat.invited = '';
            chat.invitee_set = false;
            ...
            chat.chat_text = null;
            chat.self_name = ChatService._self_reference;
            chat.header_color = ChatService._header_color;
            chat.time_reference = new Date().getTime();
        ...
        ChatService.__buildChatSession = function(scope, to_user, isopen, isfocus, store_length) {
            ...
            ChatService.__retrieveSessionKey(scope, chatSession, to_user);
            ...
            if (angular.isDefined(to_user.name)) {
                chatSession.to_user_name = to_user.name;
                var name_split = to_user.name.match(/\S+/g);
                chatSession.to_user_f_name = name_split[0];
            }
            if (angular.isDefined(isfocus)) {
                chatSession.isTextFocus = isfocus;
            } else {
            ...
            chatSession.active_session_location = ChatService.__setActiveSessionLocation(scope, to_user.user_id, false);
            ...
            ChatService.__setToUserChatPresenceLocation(chatSession, to_user);
            chatSession.to_user_message_location = ChatService.__returnToUserMessageLocation();
            chatSession.active_typing_to_user_location = ChatService.__set_active_typing_to_user_location();
            chatSession.active_typing_to_user_location.update({
                'is-typing': false
            });
            chatSession.active_session_to_user_location = ChatService.__set_active_session_to_user_location();
            ...
            chatSession.user_message_location = ChatService.__returnFromUserMessageLocation(scope);
            ...
            chatSession.active_typing_user_location = ChatService.__set_active_typing_user_location(scope);
            chatSession.active_typing_user_location.update({
                'is-typing': false
            });
            chatSession.active_typing_user_location.onDisconnect().remove();
            chatSession.active_typing_user_socket = ChatService.__set_active_typing_user_socket(scope);
            ...
            $timeout(function() {
                ...
                chatSession.messageListQuery = ChatService.__returnMessageListQuery(scope, chatSession.time, chatSession.user_message_location, ChatService._to_user._user_id, chatSession.isGroupChat, store_length);
                ...
                ChatService.__pushChatSession(chatSession, to_user, scope);
            }, 500);
        }
        ChatService.__buildGroupChatSession = function(scope, value, isopen, isfocus, store_length) {
            ...
            groupChatSession.messageListQuery = ChatService.__returnMessageListQuery(scope, groupChatSession.time, groupChatSession.group_message_location, groupChatSession.session_key, groupChatSession.isGroupChat, 1);
            groupChatSession.active_session_location = ChatService.__setActiveSessionLocation(null, groupChatSession.session_key, true);
            groupChatSession.group_active_typing_location = ChatService.__set_active_typing_group_location(scope, groupChatSession.session_key);
            ...
            groupChatSession.group_user_location.on('child_removed', function(childSnapshot) {
            ...
            if (angular.isDefined(groupChatSession.session_key) && angular.isDefined(groupChatSession.user_log) && angular.isDefined(groupChatSession.user_details)) {
                ...
                $timeout(function() {
                    ChatService.__pushChatSession(groupChatSession, false, scope);
                }, 500)
            } else {
        ...
        ChatService.__convertToGroupChat = function(chat, index, scope) {
            ...
            groupChatSession.group_active_typing_location = ChatService.__set_active_typing_group_location(scope, chat.session_key);
            groupChatSession.group_message_location = new Firebase(ChatService._group_url_root + ChatService._group_active_session_reference + chat.session_key + '/' + ChatService._group_message_location_reference);
            ...
            groupChatSession.messageListQuery = ChatService.__returnMessageListQuery(scope, chat.time, groupChatSession.group_message_location, chat.session_key, true, 1);
            ...
            groupChatSession.group_user_location.once('value', function(snapshot) {
                if (angular.isUndefined(snapshot)) {
                    ...
                    if (angular.isDefined(snapshot.val()) && snapshot.val() !== null) {
                        ...
                        angular.forEach(groupChatSession.user_log, function(val, key) {
                            var user = {};
                            user.user_id = key;
                            user.name = val.name;
                            ...
                            $timeout(function() {
                                if (user.profile_location.$value == true) {
            ...
            groupChatSession.group_user_location.on('child_added', function(snapshot) {
                if (angular.isDefined(snapshot.val()) && snapshot.val() !== null) {
                    ...
                    if (angular.isDefined(groupChatSession.user_log) && angular.isUndefined(groupChatSession.user_log[val])) {
                        ...
                        user.profile_location = UserService.__setProfileOnlineLocationforUser(val);
                        $timeout(function() {
                            if (user.profile_location.$value) {
                                user.session_location = ChatService.__setSessionLocationforGroupInvitee(groupChatSession.session_key, val);
                                groupChatSession.user_details[val] = User;
                                groupChatSession.participant_log['user_' + user.user_id] = user.name;
                                UtilityService.removeByAttr(groupChatSession.invite_list, 'user_id', val);
            ...
            if (angular.isDefined(groupChatSession.session_key) && angular.isDefined(groupChatSession.user_log) && angular.isDefined(groupChatSession.user_details)) {
                ...
                ChatService._is_playing_sound = true;
                $timeout(function() {
                    NotificationService.__playSound(NotificationService._chat_convert);
                    ChatService._is_playing_sound = false;
        ...
        ChatService.__setActiveSessionLocation = function(scope, to_user, isGroupChat) {
            ...
            if (isGroupChat === false) {
                active_session_location.on('value', function(snapshot) {
                    if (snapshot.val() == null) {
                        var removed_index = -1;
                        angular.forEach(scope.activeChats, function(value, key) {
                    ...
                    if (angular.isUndefined(snapshot)) {
                        ...
                        if (angular.isObject(snapshot.val())) {
                            ...
                            if (index == null) {
                                ...
                                index = chat_log.indexOf(data.user_id);
                            }
                            if (angular.isDefined(scope.activeChats[index])) {
                                ...
                                if (scope.activeChats[index].nudge == false && data.nudge == true) {
                                    ...
                                    $timeout(function() {
                                        ChatService._active_sessions_user_location.child(scope.activeChats[index].to_user_id).update({
                                            nudge: false
                                        });
                                    }, 5000)
                                }
        ...
        ChatService.__set_active_typing_group_location = function(scope, session_key) {
            ...
            active_typing_group_location.on('value', function(snapshot) {
                ...
                var index = log.indexOf(active_typing_group_location.parent().name());
                if (angular.isDefined(scope.activeChats[index])) {
            ...
            return active_typing_group_location;
        }
        ...
        ChatService.__returnToUserMessageLocation = function() {
            ...
            return to_user_message_location;
        }
                ...
                messageListQuery.endAt().limit(1).on('value', function(snapshot) {
                    ...
                    if (index > -1) {
                        if (scope.activeChats[index] && scope.activeChats[index].isGroupChat == false) {
                            var i = scope.activeChats[index].chats.length;
                            while (i--) {
                            ...
                            var i = scope.activeChats[index].group_chats.length;
                            while (i--) {
                        ...
                        messageListQuery.on('child_added', function(snapshot) {
                            if (snapshot.name() != ChatService.last_chat_logged) {
                                ...
                                if (index == null) {
                                    ...
                                    index = active_chat_log.indexOf(lookup);
                                }
                                if (index > -1) {
                                    if (angular.isDefined(scope.activeChats[index])) {
                                        ...
                                        scope.activeChats[index].chat_log.push(data.author);
                                    }
                        ...
                        messageListQuery.on('child_added', function(snapshot) {
                            if (snapshot.name() != ChatService.last_chat_logged) {
                                ...
                                if (index == null) {
                                    ...
                                    index = active_chat_log.indexOf(data.to);
                                }
                                if (index > -1) {
                                    if (angular.isDefined(scope.activeChats[index])) {
                                        if (scope.activeChats[index].first_priority === false) {
                                        ...
                                        scope.activeChats[index].group_chat_log.push(data.author);
                                        scope.activeChats[index].group_chats.push(data);
                                    }
                                    if ((parseInt(data.time) + 5000) > scope.activeChats[index].time_reference && data.author != UserService._user_profile.user_id) {
                                        scope.alertNewChat(index, false, data);
                                    }
        ...
        ChatService.__fetchMoreMessages = function(scope, index, location, endAt) {
            ...
            location.endAt(endAt).limit(ChatService._message_fetch_size).once('value', function(snapshot) {
                ...
                if (prev) {
                    ...
                    $timeout(function() {
                        angular.forEach(scope.temp_chats, function(value) {
        ...
        ChatService.__establishUserChat = function(scope) {
            ...
            var check_for_user = setInterval(function() {
                ...
                if (angular.isDefined(UserService._user_profile.user_id)) {
                    ...
                    ChatService._sub_category_reference = UserService._user_profile.user_id + '/';
                    ChatService.__set_active_sessions_user_location(scope);
                }
            }, 250);
        ...
        ChatService.__pushChatSession = function(chatSession, to_user, scope) {
            if (angular.isUndefined(chatSession.session_key)) {
                chatSession = null;
                return false;
            }
            if (to_user === false) {
                    ...
                    $timeout(function() {
                        ...
                        $timeout(function() {
                            if (!(ChatService._is_playing_sound) && (chatSession.time + 5000) > new Date().getTime()) {
                                if (scope.is_external_window.$value && scope.isExternalInstance == false) {} else {
                                    NotificationService.__playSound(NotificationService._new_chat);
                                }
                                $timeout(function() {
                                    ChatService._last_pushed_session = null;
                ...
                if (scope.active_sessions['user_' + to_user.user_id] != true && !(angular.equals(to_user.user_id, ChatService._last_pushed_session))) {
                    ChatService._last_pushed_session = to_user.user_id;
                    var to_user_info = {
                        avatar: to_user.avatar,
                        groupChat: chatSession.isGroupChat,
                        'session_key': chatSession.session_key,
                    ...
                    if (scope.layout != 2) {
                        if (chatSession.isTextFocus) {
                            scope.activeChats.unshift(chatSession);
                            scope.safeApply(function() {
                                $timeout(function() {
                                    scope.setDirectoryChat(0, true);
                                }, 50)
        ...
        DirectoryChatService._parent_category_reference = "Chat-System/Group-Chats" + '/';
        DirectoryChatService._is_typing_reference = 'is-typing';
        DirectoryChatService._group_message_reference = "stored-messages" + '/';
        DirectoryChatService._group_active_users_reference = 'active-users/';
        DirectoryChatService._topic_reference = 'topic/';
        DirectoryChatService._store_time = 86400000;
        ...
        DirectoryChatService.__setUrlRootReference = function() {
            DirectoryChatService._url_root = FBURL + DirectoryChatService._parent_category_reference + DirectoryChatService._directory_chat_reference.split('_')[0] + '/' + DirectoryChatService._directory_chat_reference;
        }
        ...
        DirectoryChatService.__buildNewDirectoryChat = function(scope, chat_reference, chat_description, admin, mandatory, watch_users, store_length) {
            ...
            newDirectoryChat.chat_description = chat_description
            ...
            if (watch_users) {
                ...
                newDirectoryChat.group_user_location.on('child_added', function(snapshot) {
                    if (angular.isDefined(snapshot.val()) && snapshot.val() !== null) {
                        var data = snapshot.val();
                        var val = data.user_id;
                ...
                newDirectoryChat.group_user_location.on('child_removed', function(childSnapshot) {
                    var data = childSnapshot.val();
                    var val = data.user_id;
                    if (angular.isDefined(newDirectoryChat.user_details) && angular.isDefined(newDirectoryChat.user_details[val])) {
                        ...
                        delete newDirectoryChat.user_log[val];
                        var n = Firebase.ServerValue.TIMESTAMP;
                        var chat_text = data.name + ' has left';
        ...
        DirectoryChatService.__returnMessageListQuery = function(scope, directory_chat, store_length) {
            ...
            $timeout(function() {
                messageListQuery.endAt().limit(1).on('value', function(snapshot) {
                    ...
                    var i = directory_chat.group_chats.length;
                    while (i--) {
            ...
            $timeout(function() {
                messageListQuery.on('child_added', function(snapshot) {
                    if (snapshot.name() != ChatService.last_chat_logged) {
                        ...
                        directory_chat.group_chat_log.push(data.author);
        ...
        return EmojiService;
    }).service('ChatRecordService', ['$http', '$log', function($http, $log) {
    ...
    'use strict';
    angular.module('chatModule.filters', []).filter('interpolate', ['version', function(version) {
        ...
        return function(input, query) {
            ...
            angular.forEach(input, function(userData, user) {
                if (utils.compareStr(user, query) || utils.compareStr(userData.name, query))
    ...
    'use strict';
    angular.module('chatModule.directives', []).directive('avatar', function() {
        return {
            restrict: "E",
